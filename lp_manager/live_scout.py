...
def preliminary_pool_evaluation(pool: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "preferred_sleeve": "CORE_INCOME" if (core_pre >= 72 and asset_quality >= 90 and age >= 90 and tvl >= 1_000_000) else "TACTICAL_CAMPAIGN" if tactical_pre >= 68 else None,
        "quality": {"asset": round(asset_quality,1), "liquidity": round(liquidity_score,1), "activity": round(activity,1), "age_days": round(age,1)},
        "risk_core": assess_pool_risk(candidate, sleeve="CORE_INCOME"),
        "risk_tactical": assess_pool_risk(candidate, sleeve="TACTICAL_CAMPAIGN"),
        "note": "Pre-score uses current live market structure only. Historical fee stability/range durability is added by Replay/Range Lab before approval.",
    }
